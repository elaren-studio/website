...
Registered in `robots.txt`. After significant page additions, submit the sitemap through the Search Console owner UI or Sitemaps API and inspect its receipt and later crawl status. Do not use the retired sitemap ping endpoint. On September 8, 2026, the additive OAuth upgrade retained all 19 original scopes and added `https://www.googleapis.com/auth/webmasters` (20 total, zero removals), with refresh and identity verified. The sitemap-index submission returned HTTP 204 at 16:49:08 UTC; the initial GET readback accepted it pending processing with zero errors and zero warnings. Current inspection shows the homepage `Crawled - currently not indexed` with the expected canonical and an August 30, 2026 last crawl; the three new content-marketing pages remain unknown to Google. Page indexing and canonical adoption require a later Google recrawl.

On September 8, Search Console also accepted individual indexing requests for the homepage, content-marketing offer, program-campaign example, and content-partner preparation guide. This confirms queue acceptance only; inspect later crawl/indexing separately.
...
The content, social, local SEO, DFW marketing, and private-practice marketing pages use `src/components/InquiryComposer.astro` and `src/lib/inquiry.ts` to submit directly through the existing FormSubmit route to `hello@elarenstudio.com`. Success requires the documented provider acknowledgement. Required fields, a honeypot, duplicate prevention, and a 20-second timeout bound the request. On uncertain failure, entered details remain available for retry or the locally prepared email/copy fallback. No automatic retry occurs. Without JavaScript, use the visible email link. The form uses POST so native fallback does not place entered details in a query string.

The route was activated September 9, 2026 and a labeled synthetic AJAX test was confirmed in the intended inbox. Repeat a clearly labeled synthetic check after material delivery changes, independently verify the received marker/source, and exclude it from business metrics. Do not describe provider acknowledgement alone as inbox delivery.

Run `node --test scripts/inquiry.test.ts` with a Node runtime supporting TypeScript stripping, plus the required validation/build. Test failure behavior using an isolated unavailable endpoint, restore the production endpoint, and rebuild before release. Fixed production-only Plausible events are `Inquiry Started`, `Inquiry Submitted`, and `Inquiry Submission Failed`; configure matching custom goals in the verified Elaren account. No entered form values belong in analytics. See `docs/SEARCH_MEASUREMENT.md`.

Bing ownership is represented by the public `msvalidate.01` tag in BaseLayout. Keep it after verification. Submit the sitemap index in the verified Elaren property. After a content deployment, use the selected-URL workflow in `docs/INDEXNOW.md`; no background automation is installed.
...
4. Push to GitHub
5. If new pages were added: submit the sitemap in Search Console and verify receipt; recheck indexing after Google recrawls
